...
class AvgMinMaxObserver(ObserverBase):
    # 当开启observer时，每次forward都会更新min-max，采用平均的方式
    """
...
class MSEObserver(ObserverBase):
    ...
    def perform_2D_search(self, x):
        # 同时搜索scale和zero_point, 优化目标仍然是量化前后的距离
        if self.ch_axis != -1:
            y = _transform_to_ch_axis(x, self.ch_axis)
        ...
        # print(best_min.shape)   #torch.Size([2304])
    ...
    def forward(self, x_orig):
        ...
        if self.one_side_dist != 'no' or self.symmetric:  # one-side distribution or symmetric value for 1-d search
            ...
            # print('perform_1D_search')
        else:  # 2-d search
            best_min, best_max = self.perform_2D_search(x)
            # print('perform_2D_search')
...
class PCTObserver_debug(ObserverBase):
    # grid search: more accurate but slow
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        super(PCTObserver_debug, self).__init__(bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        self.p = 2.0
        self.plist = [0.99,0.999,0.9999,0.99999]
        # self.num = 100  # candidate num
        # self.one_side_dist = None  # 'pos', 'neg', 'no'
    ...
    def perform_search(self, x):
        ...
        for pct in self.plist:
            if self.ch_axis != -1:
                try:
                    new_max = torch.quantile(x_clone, pct, dim=1)
                    new_min = torch.quantile(x_clone, 1.0 - pct, dim=1)
                except:
                    new_max = torch.tensor(np.percentile(
                        x_clone.reshape(-1).cpu(), pct * 100, axis=1),
                        device=x_clone.device,
                        dtype=torch.float32,)
                    new_min = torch.tensor(np.percentile(
                        x_clone.reshape(-1).cpu(), (1 - pct) * 100, axis=1),
                        device=x_clone.device,
                        dtype=torch.float32)
            else:
                try:
                    ...
                    new_min = torch.tensor(np.percentile(
                        ...
                        dtype=torch.float32)
            # x_q = self.quantize(x_clone, new_max, new_min)
            score = self.loss_fx(x.clone().detach(), new_min, new_max)
            # if score < best_score:
            #     best_min = new_min
            #     best_max = new_max
            best_min = torch.where(score < best_score, new_min, best_min)
...
class PCTObserver_CW(ObserverBase):
    # grid search: more accurate but slow
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        super(PCTObserver_CW, self).__init__(bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        self.p = 2.0
        self.plist = [0.99,0.999,0.9999,0.99999]
        # self.num = 100  # candidate num
        # self.one_side_dist = None  # 'pos', 'neg', 'no'
    ...
    def quantize_test(self, x, max, min):
        delta = (max - min) / (2 ** self.bit - 1)
        zero_point = (- min / delta).round()
        # we assume weight quantization is always signed
        x_int = torch.round(x / delta)
        x_quant = torch.clamp(x_int + zero_point, 0, self.quant_max)
        x_float_q = (x_quant - zero_point) * delta
        return x_float_q
    
    def lp_loss_simple(self, pred, tgt, p=2.0, reduction='none'):
        """
    ...
    def perform_search(self, x, channel_wise: bool = False):
        if channel_wise:
            x_clone = x.clone().detach()
            n_channels = x_clone.shape[self.ch_axis]
            if self.ch_axis == 0:
                if len(x.shape) == 2:
            ...
            # print(x_max.shape)
            # exit()
            best_min = torch.zeros_like(x_max)
            best_max = x_max.clone()
            for c in range(n_channels):
                best_min[c], best_max[c] = self.perform_search(x_clone[c], channel_wise=False)
            ...
            for pct in self.plist:
                try:
                    ...
                    new_min = torch.tensor(np.percentile(
                        ...
                        device=x_clone.device,
                        dtype=torch.float32)   
                x_q = self.quantize_test(x_clone, new_max, new_min)
                score = self.lp_loss_simple(x_clone, x_q, p=2, reduction='all')
                if score < best_score:
                    best_score = score
                    best_min = new_min
                    best_max = new_max
    ...
    def forward(self, x_orig):
        ...
        best_min, best_max = self.perform_search(x, channel_wise=self.ch_axis != -1)
        self.min_val = torch.min(self.min_val, best_min)
        self.max_val = torch.max(self.max_val, best_max)
