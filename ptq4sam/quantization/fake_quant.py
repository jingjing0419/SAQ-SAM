...
class QuantizeBase(nn.Module):
    ...
    def __init__(self, observer=ObserverBase, bit=8, symmetric=False, ch_axis=-1):
        ...
        self.drop_prob = 1.0
        self.clip_input = 0
        self.clip_input_l = None
        self.clip_input_r = None
...
class FixedFakeQuantize(QuantizeBase):
    ...
    def update_sz(self, min_val, max_val):
        self.observer.min_val[0] = min_val
        self.observer.max_val[0] = max_val
...
class FixedFakeQuantize4test(QuantizeBase):
...
class LSQFakeQuantize(QuantizeBase):
    # fixed Quant的scale和zero point都是固定的buffer，而LSQ的scale是可以训练的parameter
    # 多了一个梯度因子，‘通常用于梯度归一化，以确保在训练过程中梯度的规模适当。’
    ...
    def modify_ch_axis(self, ch_axis=-1):
        self.ch_axis = ch_axis
        self.observer.ch_axis = ch_axis
    
    def update_sz(self, min_val, max_val):
        self.observer.min_val = torch.tensor(min_val, device='cuda:0',dtype=torch.float32)
        self.observer.max_val = torch.tensor(max_val, device='cuda:0',dtype=torch.float32)
    ...
    def forward(self, X, value=None):
        if self.clip_input==1:
            X = torch.clamp(X, self.clip_input_l, self.clip_input_r)
        ...
        if self.fake_quant_enabled == 1:
            ...
            # self.drop_prob.to(X.dtype)
            # drop_prob_tensor = torch.tensor(self.drop_prob, dtype=X.dtype, device=X.device)
            if self.drop_prob < 1.0:
                x_prob = torch.where(torch.rand_like(X) < self.drop_prob, X, x_orig)
                return x_prob
        if self.only_clip==1:
            x_clip = torch.clamp(X, self.observer.min_val, self.observer.max_val)
            # print('clip...')
            return x_clip
        return X
...
class AdaRoundFakeQuantize(QuantizeBase):
    ...
    """
    # 通过mask来控制舍入（向上或向下）。首先，取x_floor，作为基础；然后学习一个alpha，映射为[0,1]的round mask；round mask与x_floor相加，新的量化值。
    # ？推理时mask取0/1，训练时mask取[0,1]?
    # 没有重建过程，是不起作用的
    def __init__(self, observer, bit=8, symmetric=False, ch_axis=-1):
        super().__init__(observer, bit=bit, symmetric=symmetric, ch_axis=ch_axis)
        self.register_buffer('scale', torch.tensor([1.0], dtype=torch.float))
...
class LogSqrt2Quantizer(QuantizeBase):
    ...
    def quantize(self, x, scale):
        ...
        x_q = torch.clamp(x_int, 0, levels - 1)
        # X = scale * 2 ** (-1 * x_q / self.tau)
        odd_mask = (x_q%2) * (sqrt(2)-1) + 1
        x_float_q = 2**(-1 * torch.ceil(x_q/2)) * odd_mask * scale
        x_float_q[softmax_mask] = torch.Tensor([0.0])
