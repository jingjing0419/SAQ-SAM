...
from .observer import MSEFastObserver, MinMaxObserver, AvgMinMaxObserver, MSEObserver, AvgMSEObserver, AvgMSEFastObserver, LogAvgMSEFastObserver, SignAvgMSEFastObserver, PCTObserver, PCTObserver_CW, PCTObserver_debug
...
ObserverDict = {
    ...
    'PCTObserver':  PCTObserver_debug,
}

# 量化器是在得到min-max以后，计算s,z（所以对于可学习的s，是在这里设计），最后执行量化过程，也即将浮点数映射到量化网格（所以对于舍入机制的操作是在这进行）
# 观察器是通过观察待量化数据，获得min-max，也就是激活裁剪范围。
FakeQuantizeDict = {
...
class QuantLinear(QuantizedOperator, nn.Linear):
    ...
    def forward(self, input, gamma = None):
        ...
        if gamma is None:
            ...
            fused_weight = weight.mul(gamma.unsqueeze(1))
            fused_bias = bias.mul(gamma)
            return F.linear(input, self.weight_fake_quant(fused_weight), fused_bias)
...
class QuantizedTransformerLayer(QuantizedModule):
    def __init__(self):
        super().__init__()
        
class QuantizedTransformerStage(QuantizedModule):
    def __init__(self):
        super().__init__()