...
from mmdet.utils import (build_ddp, build_dp, compat_cfg, get_device,
                         ...
                         update_data_root)
import sys
# sys.path.append("/nvme/lvchengtao/ptq4sam")
sys.path.append("/home/zhangjing/PTQ4SAM_Proj/PTQ4SAM")
import copy
import logging
import torch.nn as nn
import torch.distributed as dist
from ptq4sam.model.quant_model import specials
from ptq4sam.model.quant_model import bimodal_adjust
from ptq4sam.quantization.state import enable_calibration_woquantization, enable_quantization, disable_all
from ptq4sam.quantization.quantized_module import QuantizedLayer, QuantizedBlock, PreQuantizedLayer, QuantizedMatMul
...
from ptq4sam.repq_utils import build_model, adapter_channel_wise
from ptq4sam.quant_rep import *
...
def scale_reparameter(args, q_model):
    with torch.no_grad():
        module_dict={}
        q_model_slice = q_model.predictor.model.image_encoder.blocks
...
def main():
    ...
    # add matmul    
    build_model(model.predictor.model)
    
    wq_params = {'n_bits': q_config.w_qconfig.bit, 'channel_wise': True}
    aq_params = {'n_bits': q_config.a_qconfig.bit, 'channel_wise': False}
    
    print('Performing initial quantization ...')
    # model = quant_image_encoder(args, model, input_quant_params=aq_params, weight_quant_params=wq_params)
    model = quant_mask_decoder(args, model, input_quant_params=aq_params, weight_quant_params=wq_params)
    set_quant_state(model, input_quant=True, weight_quant=False)
    model.cuda()
    model.eval()
    with torch.no_grad():
...
def quantize_model(model, config_quant, args):
    ...
    if args.quant_encoder:
        model.predictor.model.image_encoder = specials[type(model.predictor.model.image_encoder)](model.predictor.model.image_encoder, config_quant.w_qconfig, config_quant.a_qconfig, config_quant.ptq4sam)

    replace_module(model.predictor.model.mask_decoder, config_quant.w_qconfig, config_quant.a_qconfig, config_quant.ptq4sam)
...
def recon_model(model, fp_model, cali_data, recon_config,brecq=False):
    if len(cali_data) > 16:
