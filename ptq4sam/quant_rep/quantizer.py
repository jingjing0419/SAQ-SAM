...
global cnt_pct
cnt_pct = {'0.999':0, '0.9999':0, '0.99999':0}
...
class UniformQuantizer(nn.Module):
    ...
    def __repr__(self):
        ...
        s = "(" + s + " inited={}, channel_wise={}, n_bit={})".format(self.inited, self.channel_wise, self.n_bits)
        return s
    ...
    def init_quantization_scale(self, x: torch.Tensor, channel_wise: bool = False):
        ...
        if channel_wise:
            ...
            if len(x.shape) == 4:       # conv2d的weight[out_channel, in_channel, kernel_size, kernel_size], 卷积层的激活[B, C, H,W]
                x_max = x_clone.abs().max(dim=-1)[0].max(dim=-1)[0].max(dim=-1)[0]
            elif len(x.shape) == 2:     # linear的weight
                x_max = x_clone.abs().max(dim=-1)[0]
            elif len(x.shape) == 3:     # Transformer的激活(B, hidden_size, C)
